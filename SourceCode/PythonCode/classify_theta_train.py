...
def run_fold(folder, tseries_fpath, min_pts, thetas, gamma_max, out_folder):
    ...
    train_fpath = os.path.join(folder, '..', '..', '..', 'Data', 'train.dat')
    cents_fpath = os.path.join(folder, '..', '..', 'cents.dat')
    assign_fpath = os.path.join(folder, '..', '..', 'assign.dat')
...
def classify_theta_trainMethod(tseries_fpath, base_folder, f1_target, results_sub_folder, gamma_max, max_k):
